...
bool check(std::string &s, std::list<int> &sequence) {
    ...
    std::unordered_map<int, char>::iterator it1 =
        base_patterns.find(sequence.front());
    if (it1 != base_patterns.end()) {
        ...
        std::unordered_map<int, std::vector<std::list<int>>>::iterator it2 =
            patterns.find(sequence.front());
        sequence.pop_front();
    ...
    while (std::getline(myfile, line)) {
        ...
        for (size_t i = 0; i < line.size(); i++) {
            ...
            if (line[i] == ':')
                ...
                if ((index > 0) && (last == -1))
                    indexes.push_back(
                        std::stoi(line.substr(index + 1, i - index - 1)));
                index = i;
            } else if ((ascii >= 48) && (ascii <= 57)) {
                last = -1;
                if (i == line.size() - 1)
                    indexes.push_back(
                        std::stoi(line.substr(index + 1, i - index)));
            } else if (line[i] == '|') {
