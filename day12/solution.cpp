...
std::pair<int, int> rotate(std::pair<int, int> wp, int deg){
    ...
    dir.first = int(round(wp.first * std::cos(theta) - wp.second * std::sin(theta)));
    dir.second = int(round(wp.first * std::sin(theta) + wp.second * std::cos(theta)));
    return dir;
}
